...
const Header = ({
  ...
  const secondaryButtonClasses = `${buttonBaseClasses} ${isDarkMode
  ...
  // Primary button (Add Class) - Theme-aware
  const primaryButtonClasses = `${buttonBaseClasses} header-btn-primary text-white shadow-sm`;

  // Accent button (Calendar) - Theme-aware
  const accentButtonClasses = `${buttonBaseClasses} header-btn-secondary text-white shadow-sm`;

  // Gradient button (AI Schedule) - Theme-aware
  const gradientButtonClasses = `${buttonBaseClasses} header-btn-gradient text-white shadow-md`;

  // Icon button (Notifications)
  const iconButtonClasses = `relative p-2.5 min-h-[44px] min-w-[44px] rounded-lg transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 inline-flex items-center justify-center ${isDarkMode
...
>>>>>>> 9e987d0bc2b5e1ee9fd668c7dba32ea25ee440fe
      ...
      <div className="min-w-0 flex-shrink-0">
        <h1 className={`text-xl md:text-2xl font-bold tracking-tight truncate ${isDarkMode ? 'text-white' : 'text-slate-900'
          }`}>
          {getViewTitle()}
        </h1>
        <p className={`text-xs md:text-sm mt-0.5 truncate ${isDarkMode ? 'text-slate-400' : 'text-slate-500'
          }`}>
          {isStudentMode
      ...
      {/* Actions Section */}
      <div className="header-actions flex items-center gap-2 md:gap-3 flex-wrap">
        ...
        <div className="relative">
          ...
          >
            ...
            {notifications.length > 0 && (
              <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-red-500 rounded-full ring-2 ring-white dark:ring-slate-800 animate-pulse"></span>
            )}
          </button>

          {/* Notification Dropdown */}
          {showNotifications && (
            <div className={`absolute right-0 top-full mt-2 w-80 rounded-xl shadow-2xl border overflow-hidden z-[100] animate-modal-enter ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
              }`}>
              {/* Dropdown Header */}
              <div className={`px-4 py-3 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-700 bg-slate-800/80' : 'border-slate-100 bg-slate-50/80'
                }`}>
                ...
                <button
                  ...
                  className={`p-1 rounded-md transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 ${isDarkMode
                  ...
                  notifications.map((notif, index) => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        {/* Status Indicator */}
                        <div className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 ring-2 ring-offset-1 ${isDarkMode ? 'ring-offset-slate-800' : 'ring-offset-white'
                          } ${notif.type === 'success'
