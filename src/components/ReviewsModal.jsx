  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="min-h-screen px-4 flex items-center justify-center">
        <div className={`relative w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-white'
          }`}>
          {/* Header */}
          ...
          <div className="p-6 max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-2">
                  {ratingDistribution.map(({ star, count, percentage }) => (
                    <div key={star} className="flex items-center gap-3">
                      ...
                      <div className={`flex-1 h-2 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-600' : 'bg-slate-200'
                        }`}>
                        <div
            ...
            {isStudentMode && (
              <form onSubmit={handleSubmit} className={`p-6 rounded-xl mb-6 ${isDarkMode ? 'bg-slate-700/30 border border-slate-600' : 'bg-blue-50 border border-blue-200'
                ...
                <div className="mb-4">
                  <label className={`text-sm font-medium block mb-2 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'
                    }`}>
                    Your Rating *
                  </label>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 rounded-lg border focus:outline-none focus:ring-2 focus:ring-amber-500 ${isDarkMode
            ...
            <div className="space-y-4">
              ...
              {reviews.length === 0 ? (
                ...
                reviews.map(review => (
                  <div
                    key={review.id}
                    className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-700/30' : 'bg-slate-50'
                      }`}
                  >
                    ...
                    <button className={`mt-2 flex items-center gap-1 text-xs ${isDarkMode ? 'text-slate-400 hover:text-blue-400' : 'text-slate-500 hover:text-blue-600'
                      }`}>
