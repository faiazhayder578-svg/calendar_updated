...
const ChangePasswordModal = ({ isOpen, closeModal, onChangePassword, isDarkMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-modal-enter transition-colors duration-300 ${
        isDarkMode ? 'bg-slate-800 text-white shadow-slate-900/50' : 'bg-white text-slate-800 shadow-slate-200'
      }`}>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            <button
              type="button"
              onClick={handleClose}
              className={`flex-1 px-4 py-3 rounded-xl font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 active:scale-[0.98] ${
                isDarkMode
